...
export async function POST(request: Request) {
  const auth = await getServerIdentity(PERMISSIONS.offerCreate)
  ...
  const subtotal = Number(
    validRows.reduce((sum, row) => sum + toNumber(row.line_total), 0).toFixed(2)
  )
  const currencyCode = String(validRows[0]?.currency ?? "TRY")

  const { data: insertedOffer, error: offerError } = await auth.supabase
    .from("offers")
    .insert({
  ...
  if (offerError || !insertedOffer) {
    return NextResponse.json({ error: offerError?.message ?? "Teklif kaydedilemedi." }, { status: 500 })
  }
  ...
  if (itemsError) {
    await auth.supabase
      .from("offers")
      .delete()
      .eq("id", insertedOffer.id)
      .eq("company_id", auth.identity.companyId)

    return NextResponse.json({ error: itemsError.message }, { status: 500 })
  }

  return NextResponse.json({ offer: insertedOffer })
}
